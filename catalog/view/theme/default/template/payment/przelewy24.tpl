<script type="text/javascript" src="catalog/view/javascript/p24/jquery-2.1.1.min.js"></script>
<script src="catalog/view/javascript/p24/bootstrap.min.js" type="text/javascript"></script>
<link href="catalog/view/css/p24/bootstrap.p24.css" rel="stylesheet" media="screen" />
<link href="<?php echo $p24_host; ?>inchtml/ajaxPayment/ajax.css" rel="stylesheet" type="text/css"/>
...
<div class="p24_payment">
    <form action="<?php echo $action; ?>" method="post" id="p24_checkout">
        ...
        <div class="p24_method">
            <?php if(count($p24_used_cards) > 0 && $oneclick): foreach ($p24_used_cards as $uc): ?>
            <label><input type="radio" name="p24_card" value="<?=$uc['ref_id'];?>">
                <div><?php if(!$p24_noimages): ?>
                    <div class="p24_method_image"
                         style="background-image: url('https://secure.przelewy24.pl/template/201312/bank/logo_<?=$uc['method_id'];?>.gif');"></div>
                    <?php endif; ?><?=$uc['card_type'];?> <?=$uc['mask'];?>
                </div>
            </label>
            <?php endforeach; endif; ?>

            <?php foreach ($p24_payment_methods as $pm): if(($pm->status || !$p24_only24at7) && in_array($pm->id, $p24_used_methods)): ?>
            <label><input type="radio" name="p24_method" value="<?=$pm->id;?>">
                <div><?php if(!$p24_noimages): ?>
                    <div class="p24_method_image"
                         style="background-image: url('https://secure.przelewy24.pl/template/201312/bank/logo_<?=$pm->id;?>.gif');"></div>
                    <?php endif; ?><?=$pm->name;?>
                </div>
            </label>
    ...
    <div class="buttons">
        <table>
            <tr>
                <td align="right"><a id="button-confirm" class="button"><span><?php echo $button_title; ?></span></a>
                </td>
                <td align="right" id="p24_raty" style="display: none">
                    <a id="button-raty" class="button" href="https://secure.przelewy24.pl/kalkulator_raty/?amount=<?php echo ceil($p24_amount/100);?>" target="_blank"><span><?=$button_calc;?></span></a>
                </td>
            </tr>
        </table>
    </div>
        ...
        <div class="modal-dialog" role="document" style="width: 320px;">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel"><?=$cardPaymentTitle;?></h4>
                </div>
