...
//Function to change image based on category image in popup menu.
...
$(document).ready(function() {
	...
	if($(window).width() > 991) {
        ...
        $(window).on('scroll', function () {
            //ADD .TIGHT
            if ($(window).scrollTop() + $(window).height() > $('.wrapper').outerHeight() + ($('footer').height() * 0.7)) {
                $('body').addClass('tight');
            } else {
...
function main() {
    (function () {
        ...
        jQuery(window).load(function() { // makes sure the whole site is loaded
            jQuery('.loading figure').fadeOut(); // will first fade out the loading animation
            jQuery('.loading-overlay').delay(350).fadeOut('slow'); // will fade out the white DIV that covers the website.
